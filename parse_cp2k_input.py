...
class CP2KInputTokenizer(transitions.Machine):
    def begin_basic_token(self, _, idx):
        self._current_token_start = idx

    def end_basic_token(self, _, idx):
        ...
        self._tokens += [(self._current_token_start, idx)]

    def begin_string_token(self, content, idx):
        self._current_token_start = idx
        self._tracking_quote_char = content[idx]

    def end_string_token(self, content, idx):
        ...
        self._tokens += [(self._current_token_start, idx+1)]

    def unterminated_string(self, _, idx):
        raise UnterminatedStringError(f"unterminated string detected, starting at {self._current_token_start}")

    def invalid_token_char(self, _, idx):
        raise InvalidTokenCharError(f"invalid character detected at index {idx} (token started in {self._current_token_start})")

    def is_not_escaped(self, content, idx):
        if idx > 0:
            return content[idx-1] != '\\'

        return True

    def is_matching_quote(self, content, idx):
        return self._tracking_quote_char == content[idx]
    ...
    def __init__(self):
        super().__init__(self,
            ...
            transitions = [
                ...
                { 'trigger': 'quote_char', 'source': 'string_token', 'dest': 'lookout',
                ...
                { 'trigger': 'quote_char', 'source': 'basic_token',
                  'before': 'invalid_token_char', 'dest': None },
    ...
    def token_iter(self, content):
        ...
        for linenr, line in enumerate(content.splitlines()):
            for idx, char in enumerate(line):
                char_map.get(char, self.token_char)(line, idx)
    ...
    def parse(self, content):
        ...
        for tokens in self._tokenizer.token_iter(content):
            ...
            if tokens[0].startswith('&'):
                ...
                if section_name == "END":
                    if tokens and tokens[0].upper() not in [e.text for e in nodes[-1].iterfind("./NAME")]:
                ...
                for sec in nodes[-1].iterfind("./SECTION"):
                    ...
                    nodes += [sec]  # add the current XML section node to the stack of nodes
                    ...
                    if repeats:
                        ...
                        if section_name in treerefs[-1]:
                            raise InvalidNameError(f"the section '{section_name}' can only be mentioned once")
                    ...
                    if param:  # validate the section parameter like a kw datatype
                        ...
                        treerefs[-1]["_"] = parse_tokens(param, tokens, "SECTION_PARAMETERS")["values"]
                ...
                for kw in nodes[-1].iterfind("./KEYWORD"):
                    try:
                        ...
                        if data["repeats"]:
                            ...
                            if data["name"] in treerefs[-1]:
                                raise InvalidNameError(f"the keyword '{token_name}' can only be mentioned once")
                    ...
                    if data["repeats"]:
                        ...
                        if "*" in treerefs[-1]:
                            raise InvalidNameError(f"the default keyword in section '...' can only be used once")
                        treerefs[-1]["*"] = data["values"]
...
with open("test01.inp", "r") as fhandle:
    tree = parser.parse(fhandle.read())
    import json
